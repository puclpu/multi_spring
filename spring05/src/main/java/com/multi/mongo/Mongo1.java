...
public class Mongo1 {
	...
	public static void main(String[] args) {
		...
		// JSON + 기능추가 ==> BSON 객체 {"name":"Garry"}
		Bson filter = eq("name", "Garry"); // name == Garry인 Bson 객체 filter

		MongoClient mongoClient = new MongoClient(new MongoClientURI("mongodb://localhost:27017/"));
/*
 * 		MongoClientURI uri = new MongoClientURI("mongodb://localhost:27017/");
 *		MongoClient mongoClient = new MongoClient(uri); // url 연결
*/
		MongoDatabase database = mongoClient.getDatabase("shop2");
		MongoCollection<Document> collection = database.getCollection("memo");
		FindIterable<Document> result = collection.find(filter);
		// FindIterable : Arraylist 상속받음.
